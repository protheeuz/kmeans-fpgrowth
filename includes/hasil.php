...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $min_support = isset($_POST['min_support']) ? floatval($_POST['min_support']) : 1; // Default 1% jika tidak diisi
    $min_confidence = isset($_POST['min_confidence']) ? floatval($_POST['min_confidence']) : 50; // Default 50% jika tidak diisi

    // Debugging output
    echo "<script>console.log('Max Support: " . $max_support . "%');</script>";
    echo "<script>console.log('Min Support: " . $min_support . "%');</script>";
    echo "<script>console.log('Min Confidence: " . $min_confidence . "%');</script>";
    ...
    if ($min_support > $max_support) { // Bandingkan langsung dengan max_support dalam persentase
        echo '<script>
            Swal.fire({
