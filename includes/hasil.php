...
$transactions = [];
$q = $con->query("SELECT p.id_penjualan, GROUP_CONCAT(p.kode_produk ORDER BY p.kode_produk) AS items 
                  FROM penjualan p 
...
$minSupport = 2; // Minimum support threshold
$tree = new FPTree();
...
$patterns = $tree->minePatterns($minSupport);
									...
									<tbody>
										...
										foreach ($patterns as $item => $patternList) {
											foreach ($patternList as $pattern) {
                                                ...
                                                <tr>
                                                    <td class="text-center">' . $no . '</td>
                                                    <td class="text-nowrap">' . htmlspecialchars(implode(", ", $pattern['pattern'])) . '</td>
